#!/usr/bin/env bash
# Runs inside LXC 129: mint an OWNER session for org bilco and confirm the
# Google OAuth start endpoint now redirects to Google (not 503).
set -euo pipefail
...
LOC=$(curl -s -o /dev/null -w '%{redirect_url}' \
  -H "Cookie: bilco_session=$TOKEN" http://localhost:3000/api/google/oauth/start)
echo "redirect host: $(echo "$LOC" | sed -E 's#(https?://[^/]+).*#\1#')"
echo "has client_id: $(echo "$LOC" | grep -qF '170746828196-' && echo yes || echo no)"
echo "redirect_uri ok: $(echo "$LOC" | grep -qF 'tworing.ai%2Fapi%2Fgoogle%2Foauth%2Fcallback' && echo yes || echo no)"
echo "calendar scope: $(echo "$LOC" | grep -qF 'auth%2Fcalendar' && echo yes || echo no)"
